-- Hardening of SECURITY DEFINER functions for Multi-Tenant Isolation
-- Generated on: 2026-07-12
-- Ensures no tenant can access another tenant's data.
...
REVOKE EXECUTE ON FUNCTION public.track_partner_referral(TEXT, TEXT, UUID, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.track_partner_referral(TEXT, TEXT, UUID, TEXT, TEXT) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.issue_employee_session(UUID, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.issue_employee_session(UUID, UUID) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.get_employee_pos_bundle(TEXT, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_employee_pos_bundle(TEXT, UUID) TO authenticated, service_role;
...
-- Manual Plpgsql Rewrite for resolve_branch_business_name
...
DECLARE
  v_name TEXT;
BEGIN
  -- Verify branch belongs to current user business
  IF p_branch_id IS NOT NULL AND NOT public.is_super_admin() AND (SELECT business_id FROM public.business_branches WHERE id = p_branch_id) IS DISTINCT FROM public.current_user_business_id() THEN
    RAISE EXCEPTION 'Accès non autorisé' USING ERRCODE = '42501';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.resolve_branch_business_name(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.resolve_branch_business_name(UUID) TO authenticated, service_role;


-- Manual Plpgsql Rewrite for resolve_business_module_info
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    b.business_type::TEXT AS activity_type,
    CASE 
      WHEN b.business_type = 'salon' THEN 'Salon de Coiffure'
      WHEN b.business_type = 'auto_parts' THEN 'Pièces Auto'
      WHEN b.business_type = 'school' THEN 'École'
      WHEN b.business_type = 'pharmacie' THEN 'Pharmacie'
      WHEN b.business_type = 'stationery' THEN 'Papeterie'
      ELSE 'Établissement'
    END AS module_label
  FROM public.businesses b
...
REVOKE EXECUTE ON FUNCTION public.resolve_business_module_info(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.resolve_business_module_info(UUID) TO authenticated, service_role;
