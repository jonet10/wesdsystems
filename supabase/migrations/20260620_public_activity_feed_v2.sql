-- ============================================================================
-- Public community activity feed v2
-- Normalizes partner/business activity naming and adds bar module coverage.
-- ============================================================================
...
SET type = 'partner_joined'
WHERE type = 'ambassador_joined';

UPDATE public.activity_feed
SET type = 'business_joined'
WHERE type = 'salon_joined';
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    v_message := format('%s a rejoint la plateforme', COALESCE(NULLIF(BTRIM(NEW.name), ''), 'Un nouvel établissement'));
...
-- ---------------------------------------------------------------------------
-- Backfill existing public activities
-- ---------------------------------------------------------------------------
...
SELECT
  'business_joined',
  format('%s a rejoint la plateforme', COALESCE(NULLIF(BTRIM(COALESCE(b.name, '')), ''), 'Un nouvel établissement')),
  'Haïti',
