...
-- ─── 16. UNIFIED STAFF LOGIN (checks all staff types) ───
...
BEGIN
  ...
  IF v_emp.id IS NOT NULL AND v_emp.password_hash IS NOT NULL AND v_emp.password_hash = crypt(p_pin, v_emp.password_hash) THEN
    SELECT branch_id, business_type INTO v_branch_id, v_business_type
...
DO $$ BEGIN RAISE NOTICE 'Unified check_staff_login RPC created'; END $$;

-- Grant execution permissions for unauthenticated and authenticated roles
GRANT EXECUTE ON FUNCTION public.check_staff_login(TEXT, TEXT) TO anon, authenticated;
