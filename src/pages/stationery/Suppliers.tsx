    ...
    <DashboardLayout role="salon_admin" title="Fournisseurs" subtitle="Partenaires Papeterie">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md p-6 sm:rounded-xl bg-white dark:bg-white text-slate-900 border-0 shadow-2xl">
          <DialogHeader className="mb-2"><DialogTitle className="text-xl font-bold text-slate-900">{editing ? "Modifier" : "Nouveau"} Fournisseur</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-2">
            <div><Label className="text-xs font-medium text-slate-600 mb-1.5 block">Nom de l'entreprise *</Label><Input value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} required className="bg-slate-100 border-slate-300 text-slate-900 focus-visible:ring-slate-500 focus-visible:bg-white transition-colors" /></div>
            
            <div><Label className="text-xs font-medium text-slate-600 mb-1.5 block">Personne de contact (Optionnel)</Label><Input value={form.contact_name} onChange={(e) => setForm({ ...form, contact_name: e.target.value })} className="bg-slate-100 border-slate-300 text-slate-900 focus-visible:ring-slate-500 focus-visible:bg-white transition-colors" /></div>
            ...
            <div><Label className="text-xs font-medium text-slate-600 mb-1.5 block">Adresse</Label><Input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="bg-slate-100 border-slate-300 text-slate-900 focus-visible:ring-slate-500 focus-visible:bg-white transition-colors" /></div>
          ...
          <DialogFooter className="mt-4 flex gap-3 sm:justify-between">
            ...
            <Button onClick={handleSave} disabled={saving || !form.company_name} className="flex-1 bg-black text-white hover:bg-slate-800">
